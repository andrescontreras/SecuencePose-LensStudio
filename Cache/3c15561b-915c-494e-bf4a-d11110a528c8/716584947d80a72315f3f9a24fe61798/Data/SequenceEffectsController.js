...
var triggerRegistrationEvent;

function checkAndRegister() {
    if (global.behaviorSystem) {
        registerTriggers();
        if (triggerRegistrationEvent) {
            triggerRegistrationEvent.enabled = false;
        }
    }
}

// Use UpdateEvent to check for behavior system initialization
triggerRegistrationEvent = script.createEvent("UpdateEvent");
triggerRegistrationEvent.bind(checkAndRegister);
